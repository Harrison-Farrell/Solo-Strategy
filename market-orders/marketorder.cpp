...
auto MarketOrder::toString() const -> std::string {
    ...
    ss << "MarketOrder" << "["
        ...
        << "prev:" << orderIdToString(mPrev_order ?
        mPrev_order->mOrder_id : OrderId_INVALID) << " "
        << "next:" << orderIdToString(mNext_order ?
        mNext_order->mOrder_id : OrderId_INVALID) << "]";
        return ss.str();
}
...
auto BestBidOffer::toString() const -> std::string {
    std::stringstream ss;
    ss << "Best Bid Offer\t{"
        << qtyToString(mBid_qty) << "@" << priceToString(mBid_price)
        << "X"
