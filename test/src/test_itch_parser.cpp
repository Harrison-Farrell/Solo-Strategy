...
class ITCHPaserTest : public ::testing::Test {
   protected:
    ...
    std::vector<uint8_t> last_test_data;
    void SetUp() override { }
    void TearDown() override { }
    void CreateTestFile(const std::vector<uint8_t>& data) {
        last_test_data = data;
    }
    ITCH::Message DecodeLastMessage() {
        return ITCH_Parser::DecodeMessage(last_test_data.data());
    }
};
...
TEST_F(ITCHPaserTest, SystemEventMessageTest) {
    ...
    Message message = DecodeLastMessage();
...
TEST_F(ITCHPaserTest, IPOQuotingPeriodUpdateMessageTest) {
    ...
    EXPECT_EQ(100, FromBigEndian(get<IPOQuotingPeriodUpdateMessage>(message).ipo_quotation_release_time));
...
TEST_F(ITCHPaserTest, LULDAuctionCollarMessageTest) {
    ...
    EXPECT_EQ(1000000, FromBigEndian(get<LULDAuctionCollarMessage>(message).auction_collar_reference_price));
    EXPECT_EQ(1050000,FromBigEndian(get<LULDAuctionCollarMessage>(message).upper_auction_collar_price));
    EXPECT_EQ(900000,FromBigEndian(get<LULDAuctionCollarMessage>(message).lower_auction_collar_price));
    EXPECT_EQ(6553600,FromBigEndian(get<LULDAuctionCollarMessage>(message).auction_collar_extension));
}
...
TEST_F(ITCHPaserTest, AddOrderMPIDAttributionMessageTest) {
    ...
    EXPECT_EQ(14410980767525,ArrayToUint48(get<AddOrderMPIDAttributionMessage>(message).timestamp));
    EXPECT_EQ(106517, FromBigEndian(get<AddOrderMPIDAttributionMessage>(message).order_reference_number));
...
TEST_F(ITCHPaserTest, OrderExecutedMessageTest) {
    ...
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderExecutedMessage>(message).order_reference_number));
...
TEST_F(ITCHPaserTest, OrderExecutedWithPriceMessageTest) {
    ...
    EXPECT_EQ(207588671094783,ArrayToUint48(get<OrderExecutedWithPriceMessage>(message).timestamp));
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderExecutedWithPriceMessage>(message).order_reference_number));
...
TEST_F(ITCHPaserTest, OrderCancelMessageTest) {
    ...
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderCancelMessage>(message).order_reference_number));
...
TEST_F(ITCHPaserTest, OrderDeleteMessageTest) {
    ...
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderDeleteMessage>(message).order_reference_number));
}
...
TEST_F(ITCHPaserTest, OrderReplaceMessageTest) {
    ...
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderReplaceMessage>(message).original_order_reference_number));
    EXPECT_EQ(280375465148160,FromBigEndian(get<OrderReplaceMessage>(message).new_order_reference_number));
...
TEST_F(ITCHPaserTest, NonCrossTradeMessageTest) {
    ...
    EXPECT_EQ(0,FromBigEndian(get<NonCrossTradeMessage>(message).stock_locate));
    EXPECT_EQ(255, FromBigEndian(get<NonCrossTradeMessage>(message).tracking_number));
    EXPECT_EQ(207588671094783,ArrayToUint48(get<NonCrossTradeMessage>(message).timestamp));
    EXPECT_EQ(280375465082880,FromBigEndian(get<NonCrossTradeMessage>(message).order_reference_number));
    EXPECT_EQ('S', get<NonCrossTradeMessage>(message).buy_sell_indicator);
    EXPECT_EQ(4278190250,FromBigEndian(get<NonCrossTradeMessage>(message).shares));
    EXPECT_EQ("AAPL    ",ArrayToString(get<NonCrossTradeMessage>(message).stock));
    EXPECT_EQ(2900000, FromBigEndian(get<NonCrossTradeMessage>(message).price));
    EXPECT_EQ(12025908428800000,FromBigEndian(get<NonCrossTradeMessage>(message).match_number));
}
...
TEST_F(ITCHPaserTest, CrossTradeMessageTest) {
    ...
    EXPECT_EQ(255,FromBigEndian(get<CrossTradeMessage>(message).tracking_number));
    EXPECT_EQ(207588671094783,ArrayToUint48(get<CrossTradeMessage>(message).timestamp));
    EXPECT_EQ(4702127773838221344,FromBigEndian(get<CrossTradeMessage>(message).shares));
    EXPECT_EQ("ZZZZZZZZ", ArrayToString(get<CrossTradeMessage>(message).stock));
    EXPECT_EQ(4278190250,FromBigEndian(get<CrossTradeMessage>(message).cross_price));
    EXPECT_EQ(4702127773838221344,FromBigEndian(get<CrossTradeMessage>(message).match_number));
...
TEST_F(ITCHPaserTest, BrokenTradeMessageTest) {
    ...
    EXPECT_EQ(255,FromBigEndian(get<BrokenTradeMessage>(message).tracking_number));
    EXPECT_EQ(207588671094783,ArrayToUint48(get<BrokenTradeMessage>(message).timestamp));
    EXPECT_EQ(4702127773838221344,FromBigEndian(get<BrokenTradeMessage>(message).match_number));
}
...
TEST_F(ITCHPaserTest, NOIIMessageTest) {
    ...
    EXPECT_EQ(207588671094783,ArrayToUint48(get<NOIIMessage>(message).timestamp));
    EXPECT_EQ(4702127773838221344,FromBigEndian(get<NOIIMessage>(message).paired_shares));
    EXPECT_EQ(6510615555426900570,FromBigEndian(get<NOIIMessage>(message).imbalance_shares));
    ...
    EXPECT_EQ(4278190250,FromBigEndian(get<NOIIMessage>(message).current_reference_price));
    EXPECT_EQ('C', get<NOIIMessage>(message).cross_type);
    EXPECT_EQ('C', get<NOIIMessage>(message).price_variation_indicator);
}

TEST_F(ITCHPaserTest, RetailPriceImprovementIndicatorMessageTest) {
    using namespace std;
    using namespace endian;
    using namespace ITCH;
    ...
    EXPECT_EQ(13,FromBigEndian(get<RetailPriceImprovementIndicatorMessage>(message).stock_locate));
    EXPECT_EQ(0,FromBigEndian(get<RetailPriceImprovementIndicatorMessage>(message).tracking_number));
    EXPECT_EQ(11234909934345,ArrayToUint48(get<RetailPriceImprovementIndicatorMessage>(message).timestamp));
    EXPECT_EQ("APPL    ",ArrayToString(get<RetailPriceImprovementIndicatorMessage>(message).stock));
    EXPECT_EQ('A',FromBigEndian(get<RetailPriceImprovementIndicatorMessage>(message).interest_flag));
}

TEST_F(ITCHPaserTest, DLCRMessageTest) {
    using namespace std;
    using namespace endian;
    using namespace ITCH;
    ...
    CreateTestFile({
        ...
        0xBC, 0xCD, 0x00, 0xFF, 0xFF, 0xFF,  // Timestamp (207588671094783)
        0x41, 0x41, 0x50, 0x4C, 0x20, 0x20, 0x20, 0x20,  // Stock
        0x4E,                                            // Open Eligibility
        0x5A, 0x5A, 0x5A, 0x5A,                          // Min allowable 
        0x41, 0x41, 0x50, 0x4C,                          // Max allowable
        0x41, 0x41, 0x50, 0x4C,                          // Near execution
        0x41, 0x41, 0x50, 0x4C, 0x20, 0x20, 0x20, 0x20,  // Execution time
        0xFF, 0x00, 0x00, 0xAA,                          // Lower Price
    ...
    EXPECT_EQ(0, FromBigEndian(get<DLCRMessage>(message).stock_locate));
    EXPECT_EQ(255, FromBigEndian(get<DLCRMessage>(message).tracking_number));
    EXPECT_EQ(207588671094783,ArrayToUint48(get<DLCRMessage>(message).timestamp));
    EXPECT_EQ("AAPL    ", ArrayToString(get<DLCRMessage>(message).stock));
    EXPECT_EQ('N', get<DLCRMessage>(message).open_eligibility_status);
    EXPECT_EQ(1094799436,FromBigEndian(get<DLCRMessage>(message).maximum_allowable_price));
    EXPECT_EQ(1094799436,FromBigEndian(get<DLCRMessage>(message).maximum_allowable_price));
    EXPECT_EQ(1094799436,FromBigEndian(get<DLCRMessage>(message).near_execution_price));
    EXPECT_EQ(4702127773838221344,FromBigEndian(get<DLCRMessage>(message).near_execution_time));
    EXPECT_EQ(4278190250,FromBigEndian(get<DLCRMessage>(message).upper_price_range_collar));
    EXPECT_EQ(4278190250,FromBigEndian(get<DLCRMessage>(message).lower_price_range_collar));
}
