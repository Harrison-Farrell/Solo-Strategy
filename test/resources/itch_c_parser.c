...
#define parse_uint16(a) bswap_16(*((uint16_t *)(a)))

#define parse_uint32(a) bswap_32(*((uint32_t *)(a)))

#define parse_uint64(a) bswap_64(*((uint64_t *)(a)))

#define parse_uint16(a) bswap_16(*((uint16_t *)(a)))

#define parse_ts(a) \
    (((uint64_t)bswap_16(*((uint16_t *)(a)))) << 32) | (uint64_t)bswap_32(*(uint32_t *)((a) + 2))
...
int main(int argc, char *argv[]) {
    ...
    unsigned char msg_type[22] = {'S', 'R', 'H', 'Y', 'L', 'V', 'W', 'K', 'J', 'h', 'A',
                                  'F', 'E', 'C', 'X', 'D', 'U', 'P', 'Q', 'B', 'I', 'N'};
        ...
        switch (t) {
            case 'S':
                if (parse_flag['S']) {
                    ...
                    fprintf(f_output[0], "%c,%u,%u,%llu.%09llu,%c\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000, m[11]);
                    total_type[0]++;
                    total++;
                }
                break;
            case 'R':
                if (parse_flag['R']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) uint32_t round_lot_size = parse_uint32(m + 21);
                    parse_issue_subtype(27) uint32_t etp_leverage_factor = parse_uint32(m + 34);
                    fprintf(f_output[1],
                            "%c,%u,%u,%llu.%09llu,%s,%c,%c,%u,%c,%c,%s,%c,%c,%c,%c,%c,%u,%c\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, stock, m[19], m[20], round_lot_size, m[25],
                            m[26], issue_subtype, m[29], m[30], m[31], m[32], m[33],
            ...
            case 'H':
                if (parse_flag['H']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) parse_reason(21)
                        fprintf(f_output[2], "%c,%u,%u,%llu.%09llu,%s,%c,%c,%s\n", t, stock_locate,
                                tracking_number, timestamp / 1000000000, timestamp % 1000000000,
            ...
            case 'Y':
                if (parse_flag['Y']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) fprintf(f_output[3], "%c,%u,%u,%llu.%09llu,%s,%c\n", t,
                                            locate_code, tracking_number, timestamp / 1000000000,
                                            timestamp % 1000000000, stock, m[19]);
                    total_type[3]++;
                    total++;
                }
                break;
            case 'L':
                if (parse_flag['L']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_mpid(11) parse_stock(15)
                        fprintf(f_output[4], "%c,%u,%u,%llu.%09llu,%s,%s,%c,%c,%c\n", t,
                                stock_locate, tracking_number, timestamp / 1000000000,
                                timestamp % 1000000000, mpid, stock, m[23], m[24], m[25]);
                    total_type[4]++;
                    total++;
                }
                break;
            case 'V':
                if (parse_flag['V']) {
                    ...
                    fprintf(f_output[5],
                            ...
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, level1 / 100000000, level1 % 100000000,
                            level2 / 100000000, level2 % 100000000, level3 / 100000000,
                            level3 % 100000000);
                    total_type[5]++;
                    total++;
                }
            ...
            case 'W':
                if (parse_flag['W']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    fprintf(f_output[6], "%c,%u,%u,%llu.%09llu,%c\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000, m[11]);
                    total_type[6]++;
                    total++;
                }
            ...
            case 'K':
                if (parse_flag['K']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) uint32_t ipo_quotation_release_time = parse_uint32(m + 19);
                    uint32_t ipo_price = parse_uint32(m + 24);
                    fprintf(f_output[7], "%c,%u,%u,%llu.%09llu,%s,%u,%c,%u.%04u\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000, stock,
                            ipo_quotation_release_time, m[23], ipo_price / 10000,
                            ipo_price % 10000);
                    total_type[7]++;
                    total++;
                }
            ...
            case 'J':
                if (parse_flag['J']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) uint32_t acrp = parse_uint32(m + 19);
                    uint32_t uacp = parse_uint32(m + 23);
                    uint32_t lacp = parse_uint32(m + 27);
                    uint32_t auction_collar_extension = parse_uint32(m + 31);
                    fprintf(f_output[8], "%c,%u,%u,%llu.%09llu,%s,%u.%04u,%u.%04u,%u.%04u,%u\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, stock, acrp / 10000, acrp % 10000, uacp / 10000,
                            uacp % 10000, lacp / 10000, lacp % 10000, auction_collar_extension);
                    total_type[8]++;
                    total++;
                }
                break;
            case 'h':
                if (parse_flag['h']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) fprintf(f_output[9], "%c,%u,%u,%llu.%09llu,%s,%c,%c\n", t,
                                            stock_locate, tracking_number, timestamp / 1000000000,
                                            timestamp % 1000000000, stock, m[19], m[20]);
                    total_type[9]++;
                    total++;
                }
                break;
            case 'A':
                if (parse_flag['A']) {
                    ...
                    parse_stock(24) uint32_t price = parse_uint32(m + 32);
                    fprintf(f_output[10], "%c,%u,%u,%llu.%09llu,%llu,%c,%u,%s,%u.%04u\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, order_reference_number, m[19], shares, stock,
            ...
            case 'F':
                if (parse_flag['F']) {
                    ...
                    parse_attribution(36)
                        fprintf(f_output[11], "%c,%u,%u,%llu.%09llu,%llu,%c,%u,%s,%u.%04u,%s\n", t,
                                stock_locate, tracking_number, timestamp / 1000000000,
                                timestamp % 1000000000, order_reference_number, m[19], shares,
                                stock, price / 10000, price % 10000, attribution);
                    total_type[11]++;
                    total++;
                }
                break;
            case 'E':
                if (parse_flag['E']) {
                    ...
                    fprintf(f_output[12], "%c,%u,%u,%llu.%09llu,%llu,%u,%llu\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000,
            ...
            case 'C':
                if (parse_flag['C']) {
                    ...
                    fprintf(f_output[13], "%c,%u,%u,%llu.%09llu,%llu,%u,%llu,%c,%u.%04u\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, order_reference_number, executed_shares,
                            match_number, m[31], execution_price / 10000, execution_price % 10000);
                    total_type[13]++;
                    total++;
                }
                break;
            case 'X':
                if (parse_flag['X']) {
                    ...
                    fprintf(f_output[14], "%c,%u,%u,%llu.%09llu,%llu,%u\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000,
            ...
            case 'D':
                if (parse_flag['D']) {
                    ...
                    uint64_t timestamp = parse_ts(m + 5);
                    uint64_t order_reference_number = parse_uint64(m + 11);
                    fprintf(f_output[15], "%c,%u,%u,%llu.%09llu,%llu\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000,
            ...
            case 'U':
                if (parse_flag['U']) {
                    ...
                    fprintf(f_output[16], "%c,%u,%u,%llu.%09llu,%llu,%llu,%u,%u.%04u\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, original_order_reference_number,
                            new_order_reference_number, shares, price / 10000, price % 10000);
                    total_type[16]++;
                    total++;
                }
                break;
            case 'P':
                if (parse_flag['P']) {
                    ...
                    fprintf(f_output[17], "%c,%u,%u,%llu.%09llu,%llu,%c,%u,%s,%u.%04u,%llu\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, order_reference_number, m[19], shares, stock,
            ...
            case 'Q':
                if (parse_flag['Q']) {
                    ...
                    parse_stock(19) uint32_t cross_price = parse_uint32(m + 27);
                    uint64_t match_number = parse_uint64(m + 31);
                    fprintf(f_output[18], "%c,%u,%u,%llu.%09llu,%llu,%s,%u.%04u,%llu,%c\n", t,
                            stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, shares, stock, cross_price / 10000,
                            cross_price % 10000, match_number, m[39]);
                    total_type[18]++;
                    total++;
                }
                break;
            case 'B':
                if (parse_flag['B']) {
                    ...
                    fprintf(f_output[19], "%c,%u,%u,%llu.%09llu,%llu\n", t, stock_locate,
                            tracking_number, timestamp / 1000000000, timestamp % 1000000000,
                            match_number);
                    total_type[19]++;
                    total++;
            ...
            case 'I':
                if (parse_flag['I']) {
                    ...
                    parse_stock(28) uint32_t far_price = parse_uint32(m + 36);
                    ...
                    fprintf(f_output[20],
                            ...
                            t, stock_locate, tracking_number, timestamp / 1000000000,
                            timestamp % 1000000000, paired_shares, imbalance_shares, m[27], stock,
                            far_price / 10000, far_price % 10000, near_price / 10000,
                            near_price % 10000, current_ref_price / 10000,
                            current_ref_price % 10000, m[48], m[49]);
                    total_type[20]++;
                    total++;
                }
                break;
            case 'N':
                if (parse_flag['N']) {
                    ...
                    uint16_t tracking_number = parse_uint16(m + 3);
                    uint64_t timestamp = parse_ts(m + 5);
                    parse_stock(11) fprintf(f_output[21], "%c,%u,%u,%llu.%09llu,%s,%c\n", t,
                                            stock_locate, tracking_number, timestamp / 1000000000,
                                            timestamp % 1000000000, stock, m[19]);
                    total_type[21]++;
                    total++;
